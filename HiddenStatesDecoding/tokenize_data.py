...
def embed_dataset_batch(model: InversionModel, batch: Dict) -> Dict:
    ...
    with torch.no_grad():
        batch["frozen_embeddings"] = model.call_embedding_model(batch['embedder_input_ids'], batch['embedder_attention_mask'])
    return batch
...
def get_tokenizer_mapping(
    ...
    """
    lm_tokenizer = transformers.AutoTokenizer.from_pretrained(lm,trust_remote_code=True)
    ...
    mapping = torch.zeros(max(lm_vocab.values())+1, dtype=torch.long)
