    ...
    def call_embedding_model(
        ...
        if self.embedder_fake_with_zeros:
            ...
            if type(input_ids) == torch.Tensor:
                input_ids = input_ids.to(self.embedder_device)
            elif type(input_ids) == list:
                input_ids = torch.tensor(input_ids).to(self.embedder_device)
            if type(attention_mask) == torch.Tensor:
                attention_mask = attention_mask.to(self.embedder_device)
            elif type(attention_mask) == list:
                attention_mask = torch.tensor(attention_mask).to(self.embedder_device)
            model_output = embedder(input_ids=input_ids, attention_mask=attention_mask,output_hidden_states=True)
        ...
        return embeddings.to(torch.float)
