...
EMBEDDER_MODEL_NAMES = [
    ...
    "gpt2-large",
    "gpt2-xl",
    "/users/shenrujia/yinhx/models/baichuan-inc/Baichuan2-7B-Chat",
    "/users/shenrujia/yinhx/models/Qwen2.5-7B-Instruct",
    "/users/shenrujia/yinhx/models/glm-4-9b-chat",
    "/users/shenrujia/yinhx/models/Qwen1.5-7B-Chat",
    "/users/shenrujia/yinhx/models/Llama-2-7b-chat-hf",
    "/users/shenrujia/yinhx/models/DeepSeek-R1-Distill-Qwen-7B",
    "/users/shenrujia/yinhx/models/Mistral-7B-Instruct-v0.1",
    "/users/shenrujia/yinhx/models/Mistral-7B-Instruct-v0.3",
    "/users/shenrujia/yinhx/models/Llama-3.1-8B-Instruct",
    "/users/shenrujia/yinhx/models/Phi-3.5-mini-instruct",
    "/users/shenrujia/yinhx/models/DeepSeek-R1-Distill-Llama-8B",
    "/users/shenrujia/yinhx/models/gemma-2-9b-it"
]
...
def load_embedder_and_tokenizer(name: str, torch_dtype: str, **kwargs):
    ...
    if name == "dpr":
        ...
        tokenizer = transformers.AutoTokenizer.from_pretrained(name)
        model.eval()
    elif "meta-llama/" in name or "llama" in name.lower() or 'minicpm' in name.lower() or 'mistral' in name.lower():
        ...
        tokenizer = model.tokenizer
    elif "baichuan-inc/" in name or "Qwen" in name or "glm" in name or "Yi" in name or "Phi" in name or "gemma" in name:
        if torch_dtype == "float32":
            torch_dtype = torch.float32
        elif torch_dtype == "float16":
            torch_dtype = torch.float16
        ...
        # if torch_dtype is not torch.float32:
        #     model.to_bettertransformer()
        tokenizer = transformers.AutoTokenizer.from_pretrained(name,trust_remote_code=True,padding_side="left")
    else:
...
def load_tokenizer(name: str, max_length: int) -> transformers.PreTrainedTokenizer:
    ...
    if tokenizer.pad_token is None or 'llama' in name.lower() or 'minicpm' in name.lower() or 'mistral' in name.lower():
        tokenizer.pad_token = tokenizer.eos_token
